...
void producer_plugin::process_transaction(const chain::packed_transaction_ptr& trx, bool persist_until_expired, eosio::chain::plugin_interface::next_function<chain::transaction_trace_ptr> next)
{
  my->on_incoming_transaction_async(trx, persist_until_expired, next);
} 
