...
namespace eosio {
   ...
   public:
      ...
      void handle_message( connection_ptr c, const difchain_message &msg);
      void handle_message( connection_ptr c, const block_header_with_merkle_path& msg);
      void handle_message( connection_ptr c, const channel_seed& msg);
      void handle_message( connection_ptr c, const icp_actions& msg);
      ...
      void update_difchaincon(const connection_ptr & c,const  chain_id_type &chain_id);
      const chain_id_type&  get_chainid();
      connection_ptr getconbyid(const  chain_id_type &chain_id);
      void broadcastdifmsg(); 
   };
...
#define peer_elog( PEER, FORMAT, ... ) \
  ...
  FC_MULTILINE_MACRO_END
   ...
   public:
      ...
      ~connection();
      void initialize();
      void update_chainid(const chain_id_type & chainid);
      const chain_id_type& get_chainid();
      ...
      struct connection_impl{
            connection_impl():m_chainid(app().get_plugin<chain_plugin>().get_chain_id()){}
            chain_id_type m_chainid;
      };
      std::unique_ptr<connection_impl> conmy;
      ...
      void close();
      void send_handshake();
      //Breeze send different chain message
      void send_difchainmsg();
   ...
   void connection::update_chainid(const chain_id_type & chainid)
   {
      //ilog("begin update_chainid------------------");
      conmy->m_chainid = chainid;
      //ilog("update_chainid success------------------");
   }

    const chain_id_type& connection::get_chainid()
    {
        return conmy->m_chainid;  
    }
   ...
   const tcp::socket& connection::get_socket()
   {
         return *socket;
   }

   void connection::send_difchainmsg()
   {
         //ilog("begin send difchain msg -----------------------");
         std::vector<signed_block_ptr> blocks;
         appbase::app().find_plugin<eoc_relay_plugin>()->analy_dif_chainmsg(blocks);
         //net_difchain_plugin::instance()->analyDifChainMsg(blocks);
         difchain_message difchainmsg;
         difchainmsg.network_version = net_version_base + net_version;
         difchainmsg.chain_id = my_impl->chain_id;
         controller& cc = app().find_plugin<chain_plugin>()->chain();
         for(auto itVec = blocks.begin(); itVec != blocks.end(); itVec++)
         {
            difchainmsg.signedblock = *(itVec->get());
            enqueue(net_message(difchainmsg));
            ilog("send difchain msg ${ex}",("ex",difchainmsg.signedblock.block_num()));
         }
         //ilog(" send difchain msg  success -----------------------");
   }
   ...
   void dispatch_manager::bcast_block (const signed_block &bsum) {
      ...
      if (( large_msg_notify && msgsiz > just_send_it_max) && !skips.empty()) {
         ...
         for (auto cp : my_impl->connections) {
            ...
            if(cp->get_chainid() != my_impl->chain_id)
            {
               continue;   
            }
   ...
   void net_plugin_impl::handle_message( connection_ptr c, const handshake_message &msg) {
      ...
      if (msg.generation == 1) {
         ...
         if( c->peer_addr.empty() || c->last_handshake_recv.node_id == fc::sha256()) {
          ...
          update_difchaincon(c,msg.chain_id);
   ...
   void net_plugin_impl::handle_message( connection_ptr c, const difchain_message &msg)
   {
         //net_difchain_plugin::instance()->recvDifChainMsg(msg);
         appbase::app().find_plugin<eoc_relay_plugin>()->recv_dif_chainmsg(msg);
   }

      void net_plugin_impl::handle_message( connection_ptr c, const block_header_with_merkle_path& msg){

      }

      void net_plugin_impl::handle_message( connection_ptr c, const channel_seed& msg)
      {

      }

      void net_plugin_impl::handle_message( connection_ptr c, const icp_actions& msg)
      {
...
//      dispatcher->bcast_transaction(md->packed_trx);
   ...
   //Breeze
   void net_plugin_impl::update_difchaincon(const connection_ptr & c, const  chain_id_type &chain_id)
      {
            //ilog("begin update_difchaincon---------------------------------");
            for(std::set< connection_ptr >::iterator iter = connections.begin();
            iter != connections.end(); iter++)
            {
                 if(*iter == NULL || iter->get()==NULL)
                        continue;
                 if(iter->get()->peer_name()== c.get()->peer_name())
                 {
                       iter->get()->update_chainid(chain_id);
                       break;
                 }
                 if(iter->get()->get_socket().remote_endpoint().address().to_string() 
                 ==  c->get_socket().remote_endpoint().address().to_string())
                 {
                       iter->get()->update_chainid(chain_id);
                       break;
                 }
            }
            //ilog("update_difchaincon success------------------------------");
      }

      const chain_id_type&  net_plugin_impl::get_chainid(){return chain_id;}
      //Breeze
      connection_ptr net_plugin_impl::getconbyid(const  chain_id_type &chain_id)
      {
             for(std::set< connection_ptr >::iterator iter = connections.begin(); 
             iter != connections.end(); iter++)
             {
                   if(iter->get()->get_chainid() == chain_id)
                        return *iter;
             }
             return 0;
      }
      //Breeze
      void net_plugin_impl::broadcastdifmsg()
      {
            //ilog("begin broad cast dif msg -------------------------");
            for(std::set< connection_ptr >::iterator iter = connections.begin();
            iter != connections.end(); iter++)
            {
                 if(iter->get()->get_chainid() != get_chainid()) 
                        iter->get()->send_difchainmsg();
            }
            //ilog("broad cast dif msg success-------------------------");
            //net_difchain_plugin::instance()->getInfo();
            //appbase::app().find_plugin<eoc_relay_plugin>()->getRelayInfo();
      } 
}
